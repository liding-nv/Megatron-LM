...
class ColocatedBridgeCommunicator:
    ...
    def _validate_grids(self):
        ...
        # Per-grid dim checks: tp/dp required. Src must have PP=1 and CP=1;
        # dest may have PP>1 and/or CP>1. When dest CP>1, dim_names must list
        # cp before dp (enforced below) so _build_rank_mappings walks get_rank_enum
        ...
        if 'cp' in self.src_grid.dim_names:
            ...
            if src_cp_size != 1:
                raise ValueError(
                    f"src CP must be 1 for ColocatedBridgeCommunicator, got {src_cp_size}"
                )

        # _build_rank_mappings assumes get_rank_enum(['tp']) yields cp varying
        # fastest for fixed dp — true only when cp appears BEFORE dp in
        # dim_names. If the caller reverses them, dp_idx advances at the
        # wrong cp level and rank_to_dest_coords is silently wrong.
        ...
        # Source (encoder) must have PP=1. Dest (LLM) may have PP>1 — the
        # communicator only maps to dest's first PP stage; non-first-stage
        # ranks still call communicate() but the result is unused.
